...
export const setupSteps: SetupStep[] = [
  ...
  { id: 'memory-recall', group: 'mymenu', title: 'Save a street setup to slot 1 (do this LAST)', why: 'Twist the mode dial to 1 and the whole camera is configured for street in an instant.', steps: ['Leave this until every other step in this checklist is ticked: the slot saves the camera exactly as it is right now, so finish your defaults first.', 'Then set the mode dial to A (your everyday street state).', 'MENU → Shooting → Shooting Mode → Camera Set. Memory.', 'Register it to slot 1. Save night to slot 2 and food to slot 3 later if you like.'] },

  // ----- DEFAULTS -----
  { id: 'def-iso', group: 'defaults', title: 'Auto ISO range + minimum shutter', why: 'Keeps shots sharp by raising ISO only after the shutter would get too slow.', steps: ['Set ISO to AUTO: MENU → Exposure/Color → ISO → ISO (or, once your buttons are set, press the left edge of the control wheel).', 'MENU → Exposure/Color → ISO → ISO Range Limit: set 100 to 12800. This is the overall ceiling, so night markets can still brighten; in good light Auto stays low on its own (roughly 100–6400 in daytime street).', 'MENU → Exposure/Color → ISO → ISO AUTO Min. SS: set 1/250 for street.'] },
  { id: 'def-focus', group: 'defaults', title: 'Focus defaults', why: 'Sensible street defaults so you are ready the moment you lift the camera.', steps: ['Focus Mode: AF-C (MENU → Focus → Focus Mode).', 'Focus Area: Wide (MENU → Focus → Focus Area).', 'Subject Recognition (the face and eye detection) is covered in the Focus fine-tuning section below.'] },
  { id: 'def-quality', group: 'defaults', title: 'File quality & helpers', why: 'Decide your file type and turn off little annoyances.', steps: ['MENU → Shooting → Image Quality/Rec → File Format: RAW & JPEG (or JPEG only to save space).', 'MENU → Setup → Sound → Audio signals: Off, for discreet street shooting.', 'MENU → Shooting → Shooting Display → Auto Review: Off, so the screen does not freeze after each shot.', 'MENU → Shooting → Shooting Display → Grid Line Display: On (Rule of 3rds), and keep a screen layout with the level gauge via the DISP button.'] },
  ...
  // ----- PHONE & WIRELESS -----
  { id: 'con-datetime', group: 'connect', title: 'Date, time & zone', why: 'So your photos are stamped with the right local time on your trip.', steps: ['MENU → Setup → Area/Date/Time Setting: check the date and your home time are right.', 'No need to change the zone by hand: once Auto Area Adjustment is on in the Creators’ App (see the Location tagging guide), the camera switches to Vietnam time from your phone automatically.'] },
  ...
  { id: 'con-wireless', group: 'connect', title: 'Bluetooth, remote & location', why: 'A low-power link for pairing, a remote shutter, and tagging where you shot.', steps: ['MENU → Network → Bluetooth → Bluetooth Function: On.', 'Pair a small Bluetooth remote here if you have one (handy for tripod night shots).', 'Geotagging is switched on in the Creators’ App (Cameras → Setup → Location Information Linkage), not a camera menu. See the Location tagging guide on the Trip tab to set it up and confirm it on each photo.'] },
  ...
  { id: 'han-delete', group: 'handling', title: 'Quick delete while reviewing', why: 'Cull bad shots fast with a double-press, no “are you sure?” every time.', steps: ['MENU → Setup → Operation Customize → Delete pressing twice: On.', 'Now a double-press of the Delete button bins a photo instantly while reviewing.'] },
